...
let current_users: string[] = ["john", "RafAy", "RafI", "Alice", "Bob"];

let new_users: string[] = ["Bilal", "RafAy", "RafI", "Charlie", "faizan"];
...
new_users.forEach((newUsername) => {
  ...
  if (current_users.map((c_user) => c_user.toLowerCase()).includes(lowercase)) {
    ...
    console.log(`The username ${newUsername} is available! Congratulations.`);
  }
});
